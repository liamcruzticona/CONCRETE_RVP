...
$\ge$ 35 MPa, 507 registros) y WORST (Strength $\le$ 25 MPa, 295 registros).
...
poco resistente, $\le$ 25 MPa, 295 registros).
...
\subsection{Red Bayesiana (DAG)}

Para orientar cada arista $X_i$---$X_j$ del MST:
\begin{enumerate}
    \item Calcular la tabla de contingencia $P(X_i, X_j)$.
    \item Encontrar el estado dominante $P(v_i^*, v_j^*) = \max P(x,y)$.
    \item Si $P(X_j{=}v_j^* \mid X_i{=}v_i^*) > P(X_i{=}v_i^* \mid X_j{=}v_j^*)$,
          orientar $X_i \to X_j$; en caso contrario, $X_j \to X_i$.
\end{enumerate}
...
\begin{table}[H]
    ...
    \midrule
    1 & Age      & 5.9416 & 3.7680 & \textbf{2.1736} & Si \\
    2 & FlyAsh   & 4.2685 & 2.7565 & \textbf{1.5120} & Si \\
    3 & FineAgg  & 4.1560 & 5.2866 & \textbf{1.1306} & Si \\
    4 & Slag     & 3.2866 & 4.1018 & \textbf{0.8152} & Si \\
    5 & CoarseAgg& 2.7330 & 3.3757 & \textbf{0.6427} & Si \\
    6 & Cement   & 3.5209 & 3.0076 & \textbf{0.5133} & Si \\
    7 & Superp   & 5.0647 & 5.0349 & 0.0297 & No \\
    8 & Water    & 2.7330 & 2.7565 & 0.0235 & No \\
    \bottomrule
    \end{tabular}
\end{table}

Age es la variable mas critica ($|\Delta|=2.17$, 4.3 veces el umbral).
En BEST esta en la periferia del arbol, en WORST en el centro. Water y
Superplasticizer son estables ($|\Delta|<0.05$).
...
\begin{table}[H]
    ...
    \midrule
    Superp $\to$ Water & Water $\to$ Superp \\
    Water $\to$ FineAgg & FineAgg $\to$ CoarseAgg \\
    CoarseAgg $\to$ Cement & Water $\to$ FlyAsh \\
    Age $\to$ Superp & Cement $\to$ Slag \\
    Water $\to$ CoarseAgg & CoarseAgg $\to$ Water \\
    Cement $\to$ FlyAsh & Age $\to$ Cement \\
    Slag $\to$ CoarseAgg & Cement $\to$ FlyAsh \\
    \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    Prim $=$ Kruskal & Si & Si \\
    Variables criticas (rVP) & \multicolumn{2}{c}{6 de 8} \\
    \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Age} ($|\Delta|=2.17$): La edad de curado es el factor mas
          discriminativo. En concreto fuerte (BEST), Age esta en la periferia
          del arbol (conectada solo a Superplasticizer). En concreto debil
          (WORST), Age se vuelve central (conectada a Cement). Esto refleja
          que el curado prolongado es crucial para alta resistencia.

    \item \textbf{Water y Superplasticizer} ($|\Delta|<0.05$): Son estables
          en ambos grupos. La relacion agua-aditivo es fundamental
          independientemente de la resistencia final. La direccion de la
          flecha si cambia: en BEST, Superp $\to$ Water; en WORST,
          Water $\to$ Superp.

    \item \textbf{FlyAsh} ($|\Delta|=1.51$): La ceniza volante cambia
          drasticamente su rol. En BEST esta cerca del centro, en WORST
          se aleja. Esto es consistente con su uso como reemplazo parcial
          del cemento en concretos de alta resistencia.
\end{itemize}
